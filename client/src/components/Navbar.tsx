...
const Navbar = () => {
    ...
    const handleSearch = (e : React.SubmitEvent) => {
        e.preventDefault()
        if(serachQuery.trim()){
            navigate(`/search?q=${encodeURIComponent(serachQuery.trim())}`)
            setSearchQuery("")
        }
    }
                ...
                onSubmit={handleSearch}
                 className="hidden sm:flex flex-1 max-w-sm text-xs sm:text-sm">
                    <div className="relative w-full">
                        ...
                        <input type="text" 
                        placeholder="Search for gorceries..."
                        value={serachQuery}
                ...
                <div className="flex items-center gap-3">
                    ...
                    <div className="relative">
                        ...
                        }
                        {
                            userMenuOpen && (
                                <>
                                 <div className="fixed insect-0 z-40 " onClick={() => setUserMenuOpen(false)}/>
                                <div className="absolute right-0 mt-2.5 w-56 bg-white rounded-xl shadow-lg border border-app-border py-2 z-50 animate-fade-in">
                                    {
                                        user && (
                                            <div  className="px-4 py-2 border-b border-app-border">
                                                ...
                                                <p className="text-xs text-zinc-50">{user?.email}</p>
                                    ...
                                    <div onClick={() => setUserMenuOpen(false)}>
                                        ...
                                        {user && <Link to='/addresses' className="dropdown-link"> <MapIcon size={16}/> Addresses</Link>  }
